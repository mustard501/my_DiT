...
- 前向扩散；\(\varepsilon\)-预测 + LEARNED_RANGE 方差；\(L=\mathrm{MSE}(\varepsilon)+\mathrm{VB}(\Sigma)\)
- 类别条件：\(c=t_{\mathrm{emb}}+y_{\mathrm{emb}}\)，adaLN-Zero 注入各 Transformer block
...
有效 batch \(B=\texttt{batch\_size}\times\lfloor\texttt{global\_batch\_size}/\texttt{batch\_size}\rfloor\)。论文 \(B=256\)、\(\mathrm{lr}=10^{-4}\)；线性缩放 \(\mathrm{lr}=10^{-4}\times B/256\)（例如 \(B=32\) 时 \(1.25\times10^{-5}\)）。
...
- `--progress_every 10` 写出 `progression.png`：行=样本，列=\(\hat x_0\)（左噪声 → 右干净）。
...
`KL_F8_DDCONFIG`：`ch=128`，`ch_mult=(1,2,4,4)`，每级 2 个 ResnetBlock，256→128→64→32。`attn_resolutions=[]`（下/上采样路径无 attention），bottleneck 保留 `AttnBlock`。Encoder 输出 \(2\times4\) 通道（均值/对数方差），`sample()` 后乘 0.18215。Decoder 对称。训练时 VAE 冻结。

256² RGB → 32×32×4 latent。`--recon` 使用 `mode()`；扩散训练使用 `sample()`。
...
3. \(c=t_{\mathrm{emb}}+y_{\mathrm{emb}}\)。时间步：256 维频率（cos 后 sin）+ SiLU MLP。标签：`Embedding(C+1, 384)`，额外一类为 CFG uncond token。
4. `DiTBlock`：无仿射 LayerNorm + MHSA + GELU(tanh) MLP。条件经 adaLN-Zero：`SiLU→Linear(d→6d)` 得到 attn/MLP 的 \(\gamma,\beta,\alpha\)。调制层与 FinalLayer 线性层零初始化，初始为恒等映射。
5. `FinalLayer`：adaLN（2d）+ Linear，unpatchify → 8×32×32（前 4 通道 \(\varepsilon\)，后 4 通道方差）。
...
\[
\hat\varepsilon_{1:3}=\varepsilon_{\mathrm{uncond}}+s\,(\varepsilon_{\mathrm{cond}}-\varepsilon_{\mathrm{uncond}})
\]
...
训练：\(T=1000\)，ADM 线性 \(\beta=\mathrm{linspace}(10^{-4},2\times10^{-2})\)。8 通道输出；LEARNED_RANGE 将 \([-1,1]\) 插值到 \([\log\tilde\beta_t,\log\beta_t]\)。VB 中 \(\varepsilon\) detach。`clip_denoised=False`。

采样均先 `make_spaced_diffusion`（ADM `SpacedDiffusion`）：从 1000 个 \(\bar\alpha\) 抽取 250 个，\(\beta'_i=1-\bar\alpha_{t_i}/\bar\alpha_{t_{i-1}}\)，再在新链上逐步反推。网络时间步由 `tmap` 映射回 \(\{0,\ldots,999\}\)。`--ddim_spacing` 未使用。

| 调用点 | 采样器 | 更新 |
|--------|--------|------|
| `--sample` / `--eval_fid` 默认 | DDPM 250 | \(\mu_\theta+\sigma_\theta z\)，使用预测 \(\Sigma\) |
| 训练 `samples_{step}.png` | DDIM 250，η=0 | 确定性，不使用 \(\Sigma\) 加噪 |
...
RTX 4090，DiT-S/2，ImageNet-100，batch 32，lr \(1.25\times10^{-5}\)，200k steps。采样：class 16（ambulance），250-step DDPM，`--cfg_scale 1.5`。未计算 FID。

<p align="center">
  <img src="docs/figs/ambulance_samples_final.png" width="48%"/>
  <img src="docs/figs/ambulance_progression.png" width="48%"/>
</p>
<p align="center"><em>ambulance：samples_final / progression（左噪声 → 右干净）</em></p>
